...
begin
  ...
  if GetInstalledVersions('LANDIS-II {#PackageName} v', installedVersions) then
    case GetArrayLength(installedVersions) of
      ...
      begin
      mesg := 'Error:  Multiple versions of this package are already installed:'#13#10;
...
begin
  if CurrentVersion.Version = '' then
    ...
    begin
    CurrVersPage := CreateCustomPage(wpWelcome,
      'Remove Current Version?',
      'Version ' + CurrentVersion.Version + ' of {#PackageNameLong} is currently installed.' +
      '  Do you want to remove it?');

    staticText := TNewStaticText.Create(CurrVersPage);
    with staticText do
      begin
      WordWrap := True;
      Width := CurrVersPage.SurfaceWidth;
      Caption := 'Only one version of {#PackageNameLong} can be installed at a time.' +
                 '  Therefore, version ' + CurrentVersion.Version + ' must first be removed' +
				 ' before version {#Version}{#ReleaseForAppName} can be installed.';
      Parent := CurrVersPage.Surface;
      end;

    CurrVers_RemoveRadioButton := TRadioButton.Create(CurrVersPage);
    with CurrVers_RemoveRadioButton do
      begin
      Top := Bottom(staticText) + ScaleY(8);
      Width := CurrVersPage.SurfaceWidth;
      Caption := 'Remove {#PackageName} ' + CurrentVersion.Version;
      Parent := CurrVersPage.Surface;
      OnClick := @OnRadioButtonClick;
      end;

    CurrVers_LeaveRadioButton := TRadioButton.Create(CurrVersPage);
    with CurrVers_LeaveRadioButton do
      begin
      Top := Bottom(CurrVers_RemoveRadioButton) + ScaleY(8);
      Width := CurrVersPage.SurfaceWidth;
      Caption := 'Leave {#PackageName} ' + CurrentVersion.Version + ' installed';
      Parent := CurrVersPage.Surface;
      OnClick := @OnRadioButtonClick;
      end;
    end;
